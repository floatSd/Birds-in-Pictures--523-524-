...
base=uint8(1-logical(imread('charsReduced.bmp')));
...
m = floor(coord(2,:)/26);
coord(1,:) = m*CHAR_HEIGHT+1;
coord(2,:) = (coord(2,:)-m*26)*CHAR_WIDTH+1;

overlay = uint8(zeros(CHAR_HEIGHT,n*CHAR_WIDTH,3));
for i=1:n
    x1 = (CHAR_WIDTH-1);
    x2 = (CHAR_HEIGHT-1);
  overlay(:, (CHAR_WIDTH*i-(CHAR_WIDTH-1)):(i*CHAR_WIDTH), :) = imcrop(base,[coord(2,i) coord(1,i) x1 x2]);
end
